	...
	public Login() {
		...
		btn.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				String query = "SELECT * FROM alesure_users2 WHERE uname = ? and upass = ?;";
				try (PreparedStatement stmt = conn.getConnection().prepareStatement(query)) {
					...
					if (rs.next()) {
						...
						//adminChk = rs.getBoolean("admin"); // get table column value
						new Tickets(adminChk); //open Tickets file / GUI interface
